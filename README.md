...
- Polymorphism - To define a method with the same name as an inherited method, but with different functionality which is a process called "method overloading". I used method overloading when I made the enemies, for instance, the Triangle enemy inherits from the Enemy class without any redefining of the inherited methods, but the Rectangle enemy has redefined the rotateSelf() and update() methods.
